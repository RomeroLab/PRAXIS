#!/usr/bin/env python3
"""Compute a 2D MDS embedding for the gfp sequence/function dataset.

This script performs classical multidimensional scaling (MDS) on the
`gfp_SeqFxnDataset.pkl` file by leveraging the equivalence between
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run MDS embedding on gfp sequences.")
    parser.add_argument(
        "--input",
        type=Path,
        default=Path("insilico_analysis/dataset_oracle/gfp/gfp_SeqFxnDataset.pkl"),
        help="Path to the SeqFxnDataset pickle file.",
    )
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("insilico_analysis/dataset_oracle/gfp/seq_vis/gfp_mds_embedding.csv"),
